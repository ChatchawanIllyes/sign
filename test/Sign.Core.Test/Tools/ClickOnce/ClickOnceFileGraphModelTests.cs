...
namespace Sign.Core.Test
{
    public sealed class ClickOnceFileGraphModelTests
    {
        ...
        public void FileGraph_Constructor_PreservesModelsAndSnapshotsCollections()
        {
            ...
            IDeployManifest deployManifest = Mock.Of<IDeployManifest>();
            IApplicationManifest applicationManifestModel = Mock.Of<IApplicationManifest>();
        ...
        [Fact]
        public void ManifestDiagnostic_Constructor_CopiesOutputMessage()
        {
            OutputMessage message = CreateOutputMessage();

            ClickOnceManifestDiagnostic diagnostic = new(message);

            Assert.Equal(message.Name, diagnostic.Name);
            Assert.Equal(message.Text, diagnostic.Text);
            Assert.Equal(message.Type, diagnostic.Type);
        }
        ...
        {
            return new ClickOnceManifestDiagnostic(CreateOutputMessage());
        }

        private static OutputMessage CreateOutputMessage()
        {
            ...
            return Assert.Single(manifest.OutputMessages.Cast<OutputMessage>());
        }
    }
}
